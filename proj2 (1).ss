...
(define getInitSets
  (lambda (NTs startSymbol setting)
    (if (eq? setting 'first) 
      (if (null? NTs)
          '()
          (if (eq? (car NTs) startSymbol)
              (cons (cons startSymbol (cons '() '())) (getInitSets (cdr NTs) startSymbol setting))
              (cons (cons (car NTs) (cons '() '())) (getInitSets (cdr NTs) startSymbol setting))))
      (if (null? NTs)
          '()
          (if (eq? (car NTs) startSymbol)
              (cons (cons startSymbol (cons '(eof) '())) (getInitSets (cdr NTs) startSymbol setting))
              (cons (cons (car NTs) (cons '() '())) (getInitSets (cdr NTs) startSymbol setting)))))
))
    ...
    (letrec ((follow
              (lambda (symbolSeq followSets trailer firstSets)
                (if (null? symbolSeq)
                    followSets
                    (if (contains? NTs (car symbolSeq))
                        (if (contains? (car (cdr (assoc (car symbolSeq) firstSets))) (epsilon))
                            (follow (cdr symbolSeq) (updateAssocList followSets (car symbolSeq) (union trailer (car (cdr (assoc (car symbolSeq) followSets))))) (union trailer (removeMatch (car (cdr (assoc (car symbolSeq) firstSets))) (epsilon))) firstSets)
                            (follow (cdr symbolSeq) (updateAssocList followSets (car symbolSeq) (union trailer (car (cdr (assoc (car symbolSeq) followSets))))) (car (cdr (assoc (car symbolSeq) firstSets))) firstSets))
...
(define getPredictSets
  (lambda (rules firstSets followSets firstFunc)
    (if (null? rules)
        '()
        (let* ((lhs (caar rules))
               (first (firstFunc (cadar rules) firstSets))
               (follow (car (cdr (assoc lhs followSets))))
               (delta '::=))
          (if (contains? first (epsilon))
            (cons (cons (cons lhs (cons delta (cons (cadar rules) '()))) (cons (union follow (removeMatch first (epsilon))) '()))(getPredictSets (cdr rules) firstSets followSets firstFunc))
            (cons (cons (cons lhs (cons delta (cons (cadar rules) '()))) (cons first '())) (getPredictSets (cdr rules) firstSets followSets firstFunc)))
    ))))
